...
router.post('/register', async (req, res) => {
    ...
    if (user) {
        ...
        // sendEmail(email, 
        //     'Confirm your email', 
        //     'Please click on the following link to confirm your email: http://localhost:3000/confirm?token=' 
        //     + token)
        res.json({token}) ;
    }
});
...
router.patch('/confirm', async (req, res) => {
    ...
    try {
        const {id} = jwt.verify(token, process.env.JWT_SECRET) ;
        const user = await prisma.user.update({where: {id}, data: {confirmed: true}}) ;
        if (user) {
            res.json(user);
